...
export default function CardHorizontal({news, variant,onClick}: NewsProps) {
    ...
    const renderOverlay = (displayClasses = "") => (
        <div className={`mt-6 w-full px-3 lg:w-1/2 ${displayClasses}`} onClick={onClick}>
            <div className="w-full h-64 bg-cover bg-center bg-no-repeat rounded p-5 flex flex-col gap-3 relative overflow-hidden"
                 style={{ backgroundImage: `url(${coverImage})` }}>

                <div className="absolute inset-0 bg-black/40 z-0"></div>
                <div className="z-10 flex flex-col h-full">
                    <h2 className="text-lg text-white font-mono h-[50%] overflow-x-auto">{news?.title}</h2>
                    <p className="text-gray-400/80 h-[50%] overflow-x-auto"> {news?.category.name || "Sem categoria"}</p>

                    <div className="flex flex-row flex-wrap w-1/2 h-full overflow-auto gap-1 mt-1">
                        {news?.tags?.map(tag => (
                            <span key={tag.id} className="bg-green-500 text-white text-xs px-3 py-0.5 rounded-full shadow-sm h-fit w-fit">
                                {tag.name}
                            </span>
                        ))}
                    </div>
                    ...
                    <div className="flex flex-row justify-between items-end h-full">
                        ...
                        <div className="flex flex-row gap-2 text-sm mb-auto mt-auto">
    ...
    return (
        <>
            ...
            <div className="hidden sm:block mt-6 w-full px-3 lg:w-1/2" onClick={onClick}>
                <div className="w-full h-64 bg-mist-50 border border-blue-400 rounded flex flex-row overflow-hidden">
                    ...
                    <div className="flex-1 flex flex-col p-5 h-full">
                        <div className="flex flex-col gap-1 h-[25%]">
                            <h2 className="text-base sm:text-lg text-gray-900 font-mono font-bold h-[50%] overflow-x-auto">{news?.title}</h2>
                            <p className="text-xs sm:text-sm text-gray-500 h-[50%] overflow-x-auto">{news?.category.name || "Sem categoria"}</p>
                        </div>
