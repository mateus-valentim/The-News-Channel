...
import {ImageActions} from "@/app/actions/image";
...
try {
  const response = await ImageActions(file);
  if (!response || !response.url) {
    throw new Error("Invalid response format from server. Missing 'url' attribute.")
  }
  ...
  throw new Error("Could not save image to server.")
}
